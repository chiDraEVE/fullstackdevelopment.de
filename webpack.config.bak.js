	...
	apply( compiler ) {
		compiler.hooks.done.tap( 'Update functions.php', function () {
			...
			fse.readFile( './functions.php', 'utf8', function ( err, data ) {
				...
				const vendorsRegEx = new RegExp(
					"/bundled-assets/vendors.+?'",
					'g'
				);
				const cssRegEx = new RegExp(
					"/bundled-assets/styles.+?'",
					'g'
				);
				...
				let result = data
					.replace(
						scriptsRegEx,
						`/bundled-assets/${ manifest[ 'scripts.js' ] }'`
					)
					.replace(
						vendorsRegEx,
					...
					.replace(
						cssRegEx,
						`/bundled-assets/${ manifest[ 'scripts.css' ] }'`
					);
...
if ( currentTask == 'build' || currentTask == 'buildWatch' ) {
	...
	config.output = {
		publicPath:
			'/wp-content/themes/fictional-university-theme/bundled-assets/',
